...
//           $resPonse = $payment->PaymentForm($request);
           $resPonse = ["code" => 200, "order_id" => 1, "payment_id" => "SauM4v1rRv", "status" => true, "message" => "Payment  Successfully"];
        if ($resPonse['status']) {
        $mail =       Mail::to($request->billing_email)->send(new OrderMailNotification(['PaymentResponse'=> $resPonse, 'CartDetails'=> Order::findOrFail($resPonse['order_id']),'Response'=> $request]));
           dd($mail);
