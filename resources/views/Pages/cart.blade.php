...
<div class="wraper">
    <section class="shopping_cart py_8">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-12 col-md-12 col-lg-8 col-xl-8" id="cart_messages">
                            ...
                            <ul class="shopping_items">
                                <li>
                                    <div class="shoping_imge">
                                        <img src="{{ url('/storage/products/'.$details['productdetails']->image.'') }}"
                                            alt="{{ $details['productdetails']->image}}">
                                    </div>
                                </li>
                                <li>
                                    ...
                                    <div class="shop_item_quantity qty-input">
                                        <div id='myform' method='POST' class='quantity update-cart-qty'>
                                            <a class='qtyminus minus qty-count qty-count--minus update-qty'
                                                field='quantity' quantity-type="minus"
                                                productoid="{{$details['productdetails']->id}}">-</a>
                                            ...
                                            <input type="hidden" name="product_price"
                                                id="product_price__{{$details['productdetails']->id}}"
                                                value="{{ $details['price']}}">
                                            <input type="hidden" name="product_quantity"
                                                id="product_quntity__{{$details['productdetails']->id}}" value="1">
                                            ...
                                            <a class='qty-plus plus qty-count qty-count--add update-qty'
                                                quantity-type="plus" field='quantity'
                                                productoid="{{ $details['productdetails']->id}}">+</a>
                                        </div>
                                    </div>
                                </li>
                                <li>
                                    <div class="shope_price">
                                        <div class="shope_p_tag"><span
                                                class="text-green-500 !leading-none">{{setting('site_currency')}}
                                                {{ $details['productdetails']->price}}</span>
                                        </div>
                                        <div class="price">
                                            <h6> <span id="product_quantity_price__{{$id}}">{{setting('site_currency')}}
                                                    {{  round($details['productdetails']->price  * $details["quantity"] ,2)   }}</span>
                                            </h6>
                                        </div>

                                        <div class="remove_price">
                                            <input type="hidden" name="delete_ajax_url" id="delete_ajax_url"
                                                value="{{ route('cart.delete' ,$id) }}">
                                            <a class="theme_btn" href="javascript:void(0)" id="remove_add_to_Cart"
                                                produc_id="{{ $id }}">Remove</a>
                                        </div>
                                    </div>
                                </li>
                            </ul>
                                ...
                                <div class="form-group">
                                    <div class="input-group">
                                        <select name="spicy_lavel" id="spicy_lavel" class="form-control"
                                            ajax_value="{{ route('cart.update_other')}}">
                                            <option value="">Choose Spice level</option>
                                            @foreach(getattribute('other') as $key=> $attribuite)
                                            <option value="{{$attribuite->attributes_name}}"
                                                {{ $attribuite->attributes_name == session('spicy_lavel')? 'selected' : '' }}>
                                                {{ $attribuite->attributes_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                ...
                <div class=" col-sm-12 col-md-12 col-lg-4 col-xl-4" id="order_details">
                    @if(session('cart'))
                    <div class="order_summary">
                        <div class="tittle_heading">
                            <h4> Order Summary </h4>
                            <p id="message"
                                mimimum_ammout="{{  round(( setting('minimum_order_for_delivery' ,0.00)) ,2) }}">Minimum
                                order amount is
                                {{ setting('site_currency')}}{{ round(( setting('minimum_order_for_delivery',0.00)) ,2) }}.00
                            </p>
                            <ul class="summary_main">
                                <li>
                                    ...
                                    <div class="s_total" id="subtotal" subtotal="{{ $subtotal }}"
                                        ...
                                        updated_route="{{route('cart.update_delivery') }}" trypeList="cart">
                                ...
                                <li>
                                    ...
                                    <div class="s_total" id="tax_total">
                                        ...
                                        <p id="totaltax" totaltax="{{ $tax_total }}">{{ setting('site_currency')}}
                                            {{ $tax_total }}</p>
                                    </div>
                                </li>
                                @php
                                ...
                                <li>
                                    ...
                                    <div class="s_subtotal">
                                        ...
                                        <input type="radio" type="cart" id="delivery_type" class="delivery"
                                            ...
                                            {{ setting('delivery_charge_within_5km' ,0.00)  == session('deliveryCost') ? 'checked' : '' }}>
                                        ...
                                        <input type="radio" type="cart" id="delivery_typecx" class="delivery"
                                            ...
                                            {{ setting('delivery_charge_outside_5km' ,0.00)  == session('deliveryCost') ? 'checked' : '' }}>
                                        <label for="delivery_typecx">Delivery Charge (Outside 5 km-15km)</label> <br>
                                    </div>
                            ...
                            <div class="cart_btn">
                                ...
                                <a href="javascript:void(0)" id="checkout_btn" class="theme_btn"
                                    login_url="{{url('user/login')}}" type="order_type">Proceed To Checkout</a>
                                @else
                                <a href="javascript:void(0)" id="checkout_btn" class="theme_btn" login_url="null"
                                    type=null>Proceed To Checkout</a>
